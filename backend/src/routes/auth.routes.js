...
router.post('/identity', requireAuth, (req, res) => {
  const existing = getIdentityValidation(req.user.sub);
  if (existing && existing.status === 'approved') {
    return res.status(409).json({ message: 'Tu identidad ya ha sido verificada.' });
  }
  const { fullName, documentId, documentType } = req.body || {};
  if (!fullName || !documentId) {
    return res.status(400).json({ message: 'Nombre completo y número de documento son requeridos.' });
  }
  if (String(fullName).length < 2 || String(fullName).length > 100) {
    return res.status(400).json({ message: 'Nombre debe tener entre 2 y 100 caracteres.' });
  }
  if (String(documentId).length < 4 || String(documentId).length > 32) {
    return res.status(400).json({ message: 'Documento debe tener entre 4 y 32 caracteres.' });
  }
  const result = submitIdentityValidation(req.user.sub, {
    fullName: String(fullName).trim(),
    documentId: String(documentId).trim(),
    documentType: String(documentType || 'ine').trim()
  });
  if (!result) return res.status(400).json({ message: 'Error al enviar validación.' });
  return res.status(201).json(result);
});

router.get('/identity', requireAuth, (req, res) => {
  const v = getIdentityValidation(req.user.sub);
  return res.json(v || { status: null });
});
