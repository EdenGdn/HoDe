...
import {
  ...
  createPayment, listPaymentsByUser, createNotification, findWorkerById, persistData
} from '../data/store.js';
...
router.post('/payments', (req, res) => {
  ...
  const worker = findWorkerById(hiring.workerId);
  const p = createPayment({ userId: req.user.sub, ...parsed.data });
  updateHiringStatus(hiring.id, 'completed');
  createNotification({ userId: req.user.sub, type: 'payment', title: 'Pago realizado', body: `Pago de $${parsed.data.amount.toLocaleString('es-MX')} MXN vía ${parsed.data.method}` });
  // Notify worker if has userId
  if (worker && worker.userId) {
    createNotification({ userId: worker.userId, type: 'payment', title: '💰 Pago recibido', body: `Recibiste un pago de $${parsed.data.amount.toLocaleString('es-MX')} MXN` });
  }
  persistData();
