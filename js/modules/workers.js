...
function initWorkers() {
  ...
  function getSealHtml(worker) {
    if (!worker.subscription || !worker.subscription.tier) return '';
    var seals = {
      plus: { icon: '⭐', label: 'HoDe Plus', css: 'plus' },
      pro: { icon: '🔥', label: 'HoDe Pro', css: 'pro' },
      master: { icon: '👑', label: 'HoDe Master', css: 'master' }
    };
    var s = seals[worker.subscription.tier];
    if (!s) return '';
    return ' <span class="worker-seal worker-seal--' + s.css + '">' + s.icon + ' ' + s.label + '</span>';
  }

  function getMasterPhotoClass(worker) {
    if (worker.subscription && worker.subscription.tier === 'master') return ' professional-card--master-photos';
    return '';
  }
  ...
  function renderGrid() {
    ...
    // Sort: subscribed workers first, by tier priority
    const tierOrder = { master: 0, pro: 1, plus: 2 };
    const sorted = filtered.slice().sort(function(a, b) {
      var aT = a.subscription && a.subscription.tier ? tierOrder[a.subscription.tier] : 9;
      var bT = b.subscription && b.subscription.tier ? tierOrder[b.subscription.tier] : 9;
      if (aT !== bT) return aT - bT;
      return 0;
    });

    grid.innerHTML = sorted.map(worker => `
      <article class="professional-card${getMasterPhotoClass(worker)}">
        ...
        <div class="professional-card__content">
          <h3 class="professional-card__name">${worker.name}${getSealHtml(worker)}</h3>
