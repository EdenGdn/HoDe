...
function initPublicProfile() {
    ...
    content.innerHTML =
      '<div class="public-profile__header">' +
        ...
        '<div class="public-profile__info">' +
          '<h3>' + escapeHtml(w.name) + (w.verified ? ' <span class="public-profile__badge" title="Verificado">✓</span>' : '') + getSealHtml(w) + '</h3>' +
  ...
  window.addEventListener('hode:openPublicProfile', function(e) {
    ...
    if (w) {
      ...
      if (window.HodeApi && w.id) {
        window.HodeApi.trackWorkerView(w.id).catch(function() {});
      }
      open(w);
    }
  });
