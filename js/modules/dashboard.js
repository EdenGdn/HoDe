...
function initDashboard() {
  ...
  function renderServices(services) {
    ...
    servicesList.innerHTML = services.map(function(svc) {
      return '<div class="dashboard__service-card">' +
        ...
        '<div class="dashboard__service-footer">' +
          '<span class=\"dashboard__service-price\">$' + Number(svc.price).toLocaleString('es-MX') + ' MXN</span>' +
    ...
    if (serviceFormWrap) {
      var maxSlots = 3;
      if (window.HodeSubscriptions && window.HodeSubscriptions.getCurrentTier()) maxSlots = 10;
      serviceFormWrap.style.display = services.length >= maxSlots ? 'none' : '';
    }
  }
  ...
  dashboard.addEventListener('click', function(e) {
    ...
    } else if (act === 'subscriptions') {
      var subPanel = document.getElementById('subscription-panel');
      if (subPanel) {
        var headerH3 = document.querySelector('.header') ? document.querySelector('.header').offsetHeight : 0;
        window.scrollTo({ top: subPanel.offsetTop - headerH3 - 20, behavior: 'smooth' });
      }
    } else if (act === 'search') {
