...
(function () {
  const API_BASE = window.HodeApiBase || 'http://localhost:4000';
  ...
  window.HodeApi = {
    ...
    // Subscriptions
    async getSubscriptionTiers() { return request('/api/workers/subscriptions/tiers', { method: 'GET' }, false, false); },
    async getRecommendedWorkers(city) { return request('/api/workers/subscriptions/recommended?city=' + encodeURIComponent(city || ''), { method: 'GET' }, false, false); },
    async getMySubscription() { return request('/api/workers/me/subscription', { method: 'GET' }, true, true); },
    async subscribe(tier) { return request('/api/workers/me/subscription', { method: 'POST', body: JSON.stringify({ tier }) }, true, true); },
    async cancelSubscription() { return request('/api/workers/me/subscription', { method: 'DELETE' }, true, true); },
    // Client Needs
    async listClientNeeds(params) {
      var sp = new URLSearchParams();
      if (params && params.search) sp.set('search', params.search);
      if (params && params.category) sp.set('category', params.category);
      var q = sp.toString();
      return request('/api/client-needs' + (q ? '?' + q : ''), { method: 'GET' }, false, false);
    },
    async listMyClientNeeds() { return request('/api/client-needs/me', { method: 'GET' }, true, true); },
    async createClientNeed(payload) { return request('/api/client-needs/me', { method: 'POST', body: JSON.stringify(payload) }, true, true); },
    async deleteClientNeed(id) { return request('/api/client-needs/me/' + id, { method: 'DELETE' }, true, true); },
    // Client Subscriptions
    async getClientSubscriptionTiers() { return request('/api/client-needs/subscriptions/tiers', { method: 'GET' }, false, false); },
    async getMyClientSubscription() { return request('/api/client-needs/me/subscription', { method: 'GET' }, true, true); },
    async clientSubscribe(tier) { return request('/api/client-needs/me/subscription', { method: 'POST', body: JSON.stringify({ tier }) }, true, true); },
    async cancelClientSubscription() { return request('/api/client-needs/me/subscription', { method: 'DELETE' }, true, true); },
    // Need Responses
    async listNeedResponses(needId) { return request('/api/client-needs/' + needId + '/responses', { method: 'GET' }, false, false); },
    async respondToNeed(needId, message) { return request('/api/client-needs/' + needId + '/responses', { method: 'POST', body: JSON.stringify({ message }) }, true, true); },
    // Worker Analytics
    async getMyAnalytics() { return request('/api/workers/me/analytics', { method: 'GET' }, true, true); },
    async trackWorkerView(id) { return request('/api/workers/' + id + '/track-view', { method: 'POST' }, false, false); },
    async trackWorkerContact(id) { return request('/api/workers/' + id + '/track-contact', { method: 'POST' }, false, false); },
    // Identity Validation
    async submitIdentity(payload) { return request('/api/auth/identity', { method: 'POST', body: JSON.stringify(payload) }, true, true); },
    async getIdentity() { return request('/api/auth/identity', { method: 'GET' }, true, true); }
  };
})();
